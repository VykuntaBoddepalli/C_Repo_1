...
expecthex (const char *hex, int len, const char *name, const char *explain)
{
   ...
   if (n != len)
      errx(1, "--%s expects %d hexadecimal byte%s %s", name, len, (len == 1) ? "" : "s", explain ? explain : "");
   return bin;
}
...
main (int argc, const char *argv[])
{
   ...
   /* Wait for card */
   unsigned char nfcid[MAXNFCID] = {0};
   unsigned char ats[MAXATS] = {0};
   int cards = 0;
   setled (s, ledwait);
...
#define df(x,...) do{if((e=df_##x(&d,__VA_ARGS__)))errx(1,"Failed "#x": %s",e);}while(0)
   ...
   unsigned char binzero[17] = {0};
   unsigned char *currentkey = binmaster ? binmaster : binzero;
