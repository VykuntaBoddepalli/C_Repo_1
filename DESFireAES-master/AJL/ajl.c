...
{                               // The value of this object as a string. NULL if not found. Note that a "null" string is a valid literal value
   if (!j || j->children)
      return NULL;
   return (char *)(j->val ? j->val : valnull);
}
...
static char *j_write_flags(const j_t root, ajl_t p, char pretty, char aclose)
{
   ...
   do
   {
      ...
      {                         // Object or array
        ...
        ajl_add(p, tag, j->val ? j->val : (unsigned char *)valnull);
      // Next
      j_t n = NULL;
...
{                               // Generate log files
   ...
   p += snprintf(p, path + sizeof(path) - p, "%06u-%s-%s", (unsigned int)tv.tv_usec, who, what);
